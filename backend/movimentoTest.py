...
# MODO ESTÁTICO FALSE: Essencial para rastrear mãos em um vídeo!
maos = mp_maos.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7)
...
# ================================
def identificar_movimento_libras(trajectory_buffer):
    """
    Analisa uma sequência de landmarks para identificar gestos de movimento como 'J' e 'Z'.
    """
    pinky_tip_trajectory = [frame.landmark[20] for frame in trajectory_buffer]
    index_tip_trajectory = [frame.landmark[8] for frame in trajectory_buffer]

    if len(pinky_tip_trajectory) > 15:
        start_point = pinky_tip_trajectory[0]
        mid_point = pinky_tip_trajectory[len(pinky_tip_trajectory) // 2]
        end_point = pinky_tip_trajectory[-1]

        vertical_movement = end_point.y - start_point.y
        horizontal_movement = start_point.x - end_point.x
        
        if vertical_movement > 0.1 and horizontal_movement > 0.05:
            if mid_point.y > start_point.y and abs(mid_point.x - start_point.x) < 0.05:
                return "J"

    if len(index_tip_trajectory) > 20:
        part1 = index_tip_trajectory[:8]
        part2 = index_tip_trajectory[8:16]
        part3 = index_tip_trajectory[16:]

        dir1_x = part1[-1].x - part1[0].x
        dir2_x = part2[-1].x - part2[0].x
        dir3_x = part3[-1].x - part3[0].x

        if dir1_x > 0.05 and dir2_x < -0.05 and dir3_x > 0.05:
            return "Z"
        if dir1_x < -0.05 and dir2_x > 0.05 and dir3_x < -0.05:
            return "Z"
...
def processar_imagem(image_np):
    """
    Processa uma única imagem, retornando a letra estática e os landmarks para análise de movimento.
    """
    ...
    return letra_identificada, landmarks_mao